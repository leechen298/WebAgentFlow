...
- **完整对外接口套件** —— 面向外部调度者的稳定 API / CLI / Skill /
  Tool 属于 M16。
- **用户数据管理** —— WebAgentFlow engine 不实现自己的 user account
  model、tenant model、credential vault、cloud-hosted user data
  management、billing 或 quotas。默认假设 WebAgentFlow 操作者已经拥有
  操作目标网页的权限。

...
- **pytest 失败** × 20 在 `tests/test_form_label_extractor.py` ——
  M9 留下的已有 bug，计划在 M10 后续"Form-label extractor
  扩展"迭代里修。
...
**范围**: `docs(m10): add iteration scaffolding, terminology, §10.7`（10 文件 / +795）
...
- `critical` [docs/iterations/m10/10.1-learned-path-persistence/review.md](./review.md:45)
...
- `nit` [docs/iterations/m10/README.md](../README.md:12)
...
- **[Should-fix] #1 empty-actions 丢弃** —— 需要用户拍板。三种选法：
  ...
  允许 actions=[] 的 LearnedPath 入库（给后续 M11.1 planner 一个
...
- [A5] `m10/README.md` 的 "approved 运行" → "`pass_gate = pass`"（commit 1 nit）
...
- `nit` 上一轮承诺的两个文档小修也没补：`m10/README.md` 仍写
  `approved`，`review.md` 顶部也没有 A6 disclaimer。`m10/README.md:12`
  / `review.md:1-5`
...
2. c1 should-fix `plan.md` API 自相矛盾：**仍有问题**。实现选了
   ...
   但 plan 文字没收口。
3. c1 nit `m10/README.md` approved 术语：**仍有问题**。
...
- **A5 / A6 小笔误** —— `m10/README.md` 的 "approved 运行"
...
- 进入 M10 后续执行包：先补 `10.1.1` 自主探索用例目录，或按
  `m10/README.md` 进入 `10.2` replay / drift detection 及后续
  主线任务。

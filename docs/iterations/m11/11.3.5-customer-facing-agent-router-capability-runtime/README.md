...
用户表达目标
-> 系统应收集上下文和页面线索
-> Router 判断下一步交给哪个工作 Agent / capability
-> Orchestrator 做代码侧裁决
-> Capability Runtime 执行应用能力
-> Result Reporter 统一回复用户
```

本轮把 11.3.5 从“聊天上下文恢复”升级为 **Customer-Facing Agent Router &
Capability Runtime / 面客 Agent 路由与能力运行时**。
...
Capability Runtime 才执行应用能力。
```

LLM 不直接控制浏览器。LLM 负责理解、路由建议和页面语义判断；代码负责状态、
scope、风险、授权、事件和真正调用能力；Learning / Replay / Runtime services
负责真实浏览器工作。
...
Capability Registry，并把工作 Agent 的职责边界写清楚。

## 本轮目标

- 定义 Customer-Facing Agent Router，和 Conversation Orchestrator 分开。
- 定义 Capability Registry / Skill Registry：应用能力菜单，不是 Agent 列表。
...
- 定义 Router -> Orchestrator -> Worker Agent -> Capability Runtime 的受控执行链。
- 定义 target resolution、risk policy、thinking policy、progress / loading 和 history
  trace 规则。

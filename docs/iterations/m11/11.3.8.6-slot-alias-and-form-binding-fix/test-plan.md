...
- `record_code` -> exact `record_code` field, not search;
- `record_name` -> `Name`;
- `record_category` -> `Category`;
- `quantity` -> `Stock quantity`.

The test must include a search input whose placeholder mentions record_code/name/category
...
- learned path supports `record_code`, `record_name`, `record_category`, `quantity`;
- execute intake emits `record_code`, `name`, `category`, `record_quantity`;
- replay receives slot overrides keyed as learned slots:
  `record_code`, `record_name`, `record_category`, `quantity`;
...
rg -n "<fixture-port>|/target-page|inventory item|External-Fixture-Provider" apps/api/app/services apps/api/app/prompts apps/cli/wagent apps/console/src packages
git diff --check
```
...
`http://127.0.0.1:<fixture-port>/target-page`, DB state policy `clean database`, approved

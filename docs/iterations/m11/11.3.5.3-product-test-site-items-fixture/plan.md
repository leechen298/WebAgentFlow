...
- 页面根节点 `data-testid="items-page"`。
- 项目名称输入框 `data-testid="item-name-input"`。
- 新增按钮 `data-testid="item-create-button"`。
- 操作状态 `data-testid="operation-status"`。
- 列表容器 `data-testid="item-list"`。
...
新增 import 和 route，保留现有 `/`、`/workspace-login`、`/workspace-home`。
...
- 页面可访问。
- 新增唯一项目名后，`item-list` 内出现该文本。
- 空输入不会新增空项目。
- `/workspace-login` 和 `/workspace-home` 仍可访问。
...
| product-test-site build evidence | Yes |
| `/items` create-item smoke evidence | Yes |
| `review.md` 更新 | Yes |

...
|---|---|---|---|
| ITK-1 | learn item name | `学习新增项目，名称叫测试项目A` | `intent=learn_operation`，slot `item_name=测试项目A` |
| ITK-2 | execute item name | `帮我新增项目，名称叫测试项目B` | `intent=execute_operation`，slot `item_name=测试项目B` |
| ITK-3 | alias 项目名 | `项目名是测试项目A` | slot 归一到 `item_name` |
| ITK-4 | alias name | `name 是测试项目A` | slot 归一到 `item_name` |
...
| ID | Case | Expected |
|---|---|---|
| RTV-1 | intake slots include `item_name` | `_fill_values_from_intake()` 输出 `{"item_name": "测试项目A"}` |
| RTV-2 | username / password regression | 现有 credential fill values 行为不破坏 |
| RTV-3 | sensitive trace | `item_name` 非敏感可保留；password 不明文进 trace |
...
| ID | Case | Expected |
|---|---|---|
| LRN-1 | fill action value 等于 `fill_values.item_name` | action JSON 写入 `value_slot=item_name` |
...
| RSC-2 | `ReplayRequest(..., slot_overrides={"item_name": "测试项目B"})` | schema 接受 |
...
| RPL-3 | override applied | step log 有 `value_slot=item_name`、`override_applied=true`、`effective_value=测试项目B` |
...
| ID | Case | Expected |
|---|---|---|
| CRT-1 | execute intake has `item_name=测试项目B` and path supports `value_slot=item_name` | mocked replay handler 收到 `slot_overrides.item_name=测试项目B` |
| CRT-2 | path lacks `value_slot=item_name` | runtime 阻断，不调用 replay handler |

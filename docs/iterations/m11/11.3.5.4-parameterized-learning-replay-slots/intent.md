...
11.3.5.3 已经在 `apps/product-test-site` 新增 `/items` 页面，为 P0 working loop
...
- `ConversationIntake` fallback 当前不抽取 `item_name`。

## 目标

本迭代要让 `/items` 新增项目路径支持最小参数化执行：

```text
用户：学习新增项目，名称叫测试项目A
系统：学到 fill value=测试项目A，并把该 fill action 绑定 value_slot=item_name

用户：帮我新增项目，名称叫测试项目B
系统：构造 slot_overrides.item_name=测试项目B
...
- 学习完成后对 LearnedPath fill action 写入 `value_slot=item_name`。
...
- DOM text check、`[data-testid='item-list']` evidence。
- `pending_choice`。
...
| 编号 | 标准 | 验收方式 |
|---|---|---|
| INT-1 | “名称叫测试项目A / B” 能抽取 `item_name` | intake unit tests |
| INT-2 | `_fill_values_from_intake()` 生成 `item_name` | runtime unit tests |
| LRN-1 | 学习后 fill action 可写入 `value_slot=item_name` | learning service tests |
...
| RUN-1 | chat runtime execute branch 把 `item_name` 传到 replay handler | runtime integration tests |
| SAFE-1 | path 无 `value_slot=item_name` 时阻断 replay | runtime integration tests |

...
- Notes：
  ...
  - fixture-site 是工程验证靶场，不适合作为 product-level chat 验收站。
  - fixture-site 应独立于 fixture-site specs / assertions。
...
- Notes：
  ...
  - 当时文档门禁已打开，允许进入 `apps/fixture-site` 和 product-level
...
- Decision：passed
- Notes：
  - M11.3.3 已实现 fixture-site、product-level chat learning 和 target URL scoped execution。
...
- “只学习和操作用户输入的站点；未学习过的站点需要给用户反馈。” -> accepted；
  实现阶段必须移除 product-level `/entry` gate，并按 target URL / site scope 匹配当前 session
  learned actions。
...
- 更新 `docs/user-guide/wagent-chat.md`，说明 fixture-site 是当前可用的产品级聊天验收站。
- 文档审核通过；后续实现已经完成并收口。
- 新增 `apps/fixture-site`，默认端口 `<fixture-port>`。
- 根 `pnpm run dev` 接入 fixture-site。
...
- Learned action：`进入工作台`，target URL `http://localhost:<fixture-port>/target-login`。
- Replay result：`replay_status=succeeded`，`final_url=http://localhost:<fixture-port>/workspace-home`，
  `drift_status=none`，`step_count=4`。
- 未学习 URL fallback：`帮我在 http://localhost:<fixture-port>/orders 导出订单` 返回
...
| `pnpm --filter @web-agent-flow/fixture-site build` | fixture-site build pass | build passed | 0 | pass | vite build output | product site |
| `pnpm --filter @web-agent-flow/fixture-site build` | fixture-site regression build pass | build passed | 0 | pass | vite build output | fixture-site retained |
...
- 如果实现阶段仍通过 `spec_id=login / scenario=valid_credentials` 完成 fixture-site learning，不得 accepted。
- 如果产品级学习读取 `apps/fixture-site/specs/*.assertions.json`，不得 accepted。
- 如果用户未在聊天中提供必要输入而系统从 validation spec 自动拿输入，不得 accepted。
- 如果 fixture-site 与 fixture-site 页面结构、文案、路由一比一复制，不得 accepted。
- 如果根目录 `pnpm run dev` 不启动 fixture-site，不得 accepted。
...
`http://localhost:<fixture-port>/target-login`，未学习 URL 走 `chat_no_path` / `unlearned_target_url`。

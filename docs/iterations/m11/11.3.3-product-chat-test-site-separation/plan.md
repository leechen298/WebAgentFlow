...
- `apps/fixture-site/` - 新增产品级 chat 验收站点。
- root workspace / scripts - 接入 fixture-site dev / build。
...
- `docs/user-guide/wagent-chat.md` - 实现后把 fixture-site 从“计划”更新为“可用”。
...
1. 文档阶段：创建 11.3.3 七件套。
   ...
   - 写清 fixture-site 与 fixture-site 边界。
   - 写清 11.3.2 编号已占用。
...
3. 文档阶段：更新用户指南。
   - 说明 fixture-site 是计划新增的产品级验收站。
   - 保持 `.venv/bin/wagent chat` 为主入口。
   - 不把 fixture-site 写成当前可用。

4. 实现阶段：新增 `apps/fixture-site`。
   - `@web-agent-flow/fixture-site`。
   - port `<fixture-port>`。
   - 第一阶段实现 `/target-login`。
...
5. 实现阶段：改造 `wagent chat` product learning。
   ...
   - 不传 `spec_id / scenario`。
   - 移除 product-level learning 的 `/entry` only gate。
...
6. 实现阶段：验证。
   - fixture-site build。
   - `pnpm run dev` 一键启动包含 fixture-site。
   - fixture-site regression build。
...
| Command | Expected proof | Live autonomous verification excluded? | Notes |
|---|---|---|---|
| `pnpm --filter @web-agent-flow/fixture-site build` | fixture-site build pass | Yes | 不触发 browser |
| `pnpm run dev` | fixture-site 随 console / api / worker / fixture-site 一起启动 | Yes | 实现阶段必须验证 |
| `pnpm --filter @web-agent-flow/fixture-site build` | fixture-site regression pass | Yes | 保留工程靶场 |
...
- [x] fixture-site 不复用 validation specs / assertions。
- [x] fixture-site 已接入根目录 `pnpm run dev`。
- [x] product-level learning 不传 `spec_id / scenario`。
- [x] product-level learning 不再限制为 `/entry`。
- [x] 学习和执行都以用户输入或当前 session 已学习的 target URL 为边界。
- [x] 未学习过的站点 / 页面返回用户级反馈，不跨站点命中历史 LearnedPath。
- [x] 修改或删除 `login.assertions.json` 不影响 fixture-site learning。
- [x] fixture-site 原有 verify / smoke 能力不被破坏。

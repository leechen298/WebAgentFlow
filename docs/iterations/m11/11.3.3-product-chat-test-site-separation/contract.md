...
`apps/fixture-site` 是工程验证靶场。它继续用于 deterministic regression：
...
本轮不得删除、弱化、迁移或重写 fixture-site。

### fixture-site

`apps/fixture-site` 是产品级聊天验收站。它用于验证普通用户通过
`wagent chat` 教 WebAgentFlow 操作网页，而不是验证工程 oracle。

推荐包名和端口：

```text
package: @web-agent-flow/fixture-site
dev port: <fixture-port>
base URL: http://localhost:<fixture-port>
```

fixture-site 必须接入项目一键启动：

```bash
pnpm run dev
```

一键启动应同时启动 console、api、worker、fixture-site 和 fixture-site。
`pnpm --filter @web-agent-flow/fixture-site dev` 可以保留为单独调试入口，但不能替代
...
该路径不得依赖 fixture-site oracle：

- 不读取 `apps/fixture-site/specs/*.assertions.json`。
- 不传 `spec_id / scenario`。
- 不从 assertions 取 inputs。
- 不硬编码 `/entry` 或 `valid_credentials`。
...
- 用户必须提供目标 URL。
- 系统只能打开并学习用户输入的 URL。
- 不得在用户未提供 URL 时默认跳到 fixture-site、fixture-site 或任何固定站点。
- 不得因为 path 不是 `/entry` 就拒绝产品级学习；产品级路径不应保留 `/entry` 特化。
...
- 不得因为存在 fixture-site 的历史 LearnedPath，就跨站点执行。
...
- 如果修改或删除 `login.assertions.json` 会影响 fixture-site 学习，不得 accepted。
- 如果根目录 `pnpm run dev` 不启动 fixture-site，不得 accepted。
- 如果产品级学习仍限制为 `/entry`，或仍返回“当前只支持学习登录页”，不得 accepted。
...
- 为 fixture-site 引入 validation `spec_id / scenario` requirement。
- 把 fixture-site 注册为 validation specs 的消费者。
- 在 product-level chat learning 中保留 `/entry` only gate。
...
- pass_gate / scorecard。
- `verify-scenario`。
- fixture-site assertions。

这两类 evidence 不得混写。没有真实 fixture-site smoke 时，不得声称产品级 chat
验收通过。
...
- `fixture-site` 原有 routes、specs、fixtures、verify / smoke 能力必须保留。
- 根目录 `pnpm run dev` 必须继续保留现有 console / api / worker / fixture-site，并新增
  fixture-site；不能为了新增产品验收站移除原有一键启动成员。
...
- 无。新站点命名为 `apps/fixture-site`，端口 `<fixture-port>`。

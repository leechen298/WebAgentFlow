...
`11.3.8.2-suggested-utterance-generation/` 当前状态为 `PACKAGE_COMPLETE`。
该包已生成完整七件套，通过 spec / design review，并实现 deterministic reusable
utterances。Focused verification：`test_learning_run_service.py` 11 passed，
`test_conversation_chat_runtime.py` 89 passed；未运行 external black-box validation。

`11.3.8.3-learned-action-matching-improvement/` 当前状态为 `PACKAGE_COMPLETE`。
该包已生成完整七件套，通过 design / safety review，实现 action-side identity matcher
consumption 和 router known-action counting alignment。Focused verification：
`test_conversation_chat_runtime.py` 95 passed，`test_conversation_router_agent.py`
15 passed；未运行 external black-box validation，`PV-CLI-003` 仍需 `11.3.8.5`
实跑后才能判定。

`11.3.8.4-regression-tests/` 当前状态为 `PACKAGE_COMPLETE`。该包新增 repo-local
target-agnostic regression，证明 wrapper learning result 经过 session action identity /
utterances / match_terms 后，可用新 slot values execute 同一业务动作并进入 replay handoff。
Focused verification：`test_learning_run_service.py` + `test_conversation_chat_runtime.py`

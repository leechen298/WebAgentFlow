...
| runtime special-case inspection | Historical pre-cleanup blocker discovery | Found likely blockers in `apps/api/app/services/conversation/intake.py`, `apps/api/app/services/conversation/chat_runtime.py`, and `apps/api/app/services/learning/learning_run_service.py`; eval runner constants remained under `scripts/evals/` | 0 | Pass | command output | Historical design review evidence; superseded by the post-cleanup forbidden scan pass below |
...
| `pnpm run eval:wagent:user-behavior -- --timeout 15` | Historical blocked preflight artifact when API was unavailable | `status=blocked`, exit `2`; API health connection refused | 2 | Blocked | `wagent-user-behavior-eval-20260523T170951Z.json` / Markdown artifact | Historical non-pass artifact; superseded by the latest pass artifact |
| `python3 .agents/skills/webagentflow-eval-integrity/scripts/eval_result_gate_check.py artifacts/wagent-user-behavior-eval/wagent-user-behavior-eval-20260523T170951Z.json` | Historical blocked JSON is not treated as pass | `decision=BLOCKED`, reason `top-level status is blocked` | 2 | Blocked | command output | Exit `2` was expected for the historical blocked artifact |

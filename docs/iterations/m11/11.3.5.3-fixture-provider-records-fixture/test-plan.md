...
本轮是代码型迭代，涉及新的 fixture-site 页面和后续 runtime 闭环页面基座，
因此必须维护 `test-plan.md`。

## 测试范围

- Unit：当前 fixture-site 无单元测试脚本；本包不强制新增测试框架。
- Integration：router + Vite build。
- Product Site UI：必须验证 `/records` 页面新增项目行为。
...
|---|---|---|---|---|---|
| Docs | DOC-1 文档包完整 | `find docs/iterations/m11/11.3.5.3-fixture-site-items-fixture -maxdepth 1 -type f` | 七件套存在 | Yes | 文档阶段 |
| Docs | DOC-2 whitespace | `git diff --check` | clean | Yes | 文档 / 实现阶段都应跑 |
| Product site | SITE-1 build | `pnpm --filter @web-agent-flow/fixture-site build` | `vue-tsc --noEmit && vite build` 通过 | Yes | 实现阶段 |
| Product site | SITE-2 route | `http://localhost:<fixture-port>/records` | 页面显示 `records-page` | Yes | `<fixture-port>` 仅本地示例 |
| Product site | SITE-3 stable anchors | Browser / DOM inspect | 必须存在 P0 `data-testid` | Yes | 后续 learning / evidence 依赖 |
| Product site | SITE-4 create unique item | 输入 `测试项目B-${timestamp}` 后点击新增 | `[data-testid='record-list']` 内出现该文本 | Yes | 后续 evidence 兼容 |
...
| Regression | REG-1 old routes | `/target-login`、`/workspace-home` | 页面仍可打开 | Yes | 不破坏 11.3.3 |
...
```text
[data-testid='record-list'] 内出现 测试项目B-<timestamp>
...
- 没有真实打开 `/records` 并新增项目，不得声称页面 smoke 通过。

## 验收记录要求

实现阶段 `review.md` 必须记录：

- `pnpm --filter @web-agent-flow/fixture-site build` 输出。
- `/records` 页面 smoke 的目标 URL。
- 用于新增的唯一 `record_name`。
- 列表中出现该 `record_name` 的证据描述。
- 未运行项及原因。

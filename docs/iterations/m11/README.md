...
- [11.3.2-chat-history-debug-console](./11.3.2-chat-history-debug-console/) —— Chat History & Debug Console：补 Conversation session list、aggregate history、Console history 页面、CLI list/history/resume，服务人工测试和 Codex / Kimi Code 调试复用。状态：ready for implementation（docs review passed with minor changes, code pending）。
...
拆分包。它不实现 product-test-site，只定义后续新增 `apps/product-test-site`、端口
`5176`，以及 product-level chat learning 不读取 validation specs、不传 `spec_id` /
`scenario` 的验收红线。它不迁移 `11.2.4.2`，也不覆盖 `11.3.2`。

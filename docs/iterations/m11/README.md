...
- [11.3.8.5-external-black-box-revalidation-closeout](./11.3.8.5-external-black-box-revalidation-closeout/) —— External Black-box Revalidation / Closeout：validation / closeout 七件套已创建；当前停在 live validation approval gate，等待 API base URL、target URL、DB state policy、scenario list、latest-result-doc update approval。状态：NEEDS_USER_INPUT。

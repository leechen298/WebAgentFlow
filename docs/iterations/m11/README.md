...
- [11.3.6.2-failure-recovery-eval](./11.3.6.2-failure-recovery-eval/) —— Failure Recovery Eval：扩展 runner 覆盖 recovery menu safety、retry / relearn / cancel 出口和 private payload safety。状态：draft_for_review（未实现代码）。
...
验收结果。
`11.3.6.2-failure-recovery-eval/` 是 11.3.6 program 的第二个执行包设计稿。它计划在

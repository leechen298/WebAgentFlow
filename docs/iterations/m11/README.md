...
- [11.1.4-task-planning-dispatch-preview](./11.1.4-task-planning-dispatch-preview/) —— Task Planning Dispatch Preview。状态：documentation initialized。
- Slot Binding contract and deterministic binding MVP —— 状态：future，尚未分配执行包编号。
- Plan confirmation and consent gate —— 状态：future，尚未分配执行包编号。

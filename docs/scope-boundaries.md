...
- **Full CLI tool for end-user task execution** — not this phase. A
  ...
  below landing the three-phase main loop. The narrow
  `apps/cli/verify_scenario.py` shipped today only backs the
  `verify-scenario` Claude Code skill (AI coding agents asking the
  engine to verify a scenario); it is not the full CLI tool.
- **Skill / Tool interface for third-party Agents** — the public
  interface is still the HTTP API. The one exception is the
  `verify-scenario` Claude Code skill (generated by
  `pnpm run skill:install` into `~/.claude/skills/`), which wraps the
  same HTTP endpoint with an auditable reporting contract so an AI
  coding agent can run scenarios without bypassing the Supervisor
  Agent. A broader registry for third-party Agents is still a
  long-term direction in `product-model.md` §10.3.

...
Reason: all approved scenarios ran against the external Fixture-Site target.
...
- Playwright browser smoke against `http://127.0.0.1:<fixture-port>/target-page`
...
| PV-CLI-002 Learn create inventory item | `PASS` | Input: `Learn how to create an inventory item with record_code REC-ALP-001, name Alpine Notebook, category Stationery, and stock quantity 24.` WAgent response included `开始学习页面操作。` and `学习完成：我学会了create inventory item操作。之后你可以说“帮我create inventory item”。` Event `chat_learning_completed` recorded alias `create inventory item` and canonical goal `create_inventory_item`. |
| PV-CLI-003 Execute learned create with new values | `PASS` | Input: `Create an inventory item with record_code REC-SKY-014, name Skyline Mug, category Office, and stock quantity 18.` WAgent response: `执行中。` then `执行完成。页面证据已确认目标值“REC-SKY-014”。` Events recorded `chat_execution_completed`, `replay_status=succeeded`, `drift_status=none`, and `verification_outcome=verified`; page evidence verified `REC-SKY-014`, `Skyline Mug`, `18`, and `Office`. |
...
| PV-INTEGRITY-001 Main repo contains no external site source | `PASS` | Main repo did not restore Fixture-Site source. External target was started from `/Users/leechen/projects/External-Fixture-Provider`. |

...
describe('AutonomousUseCasesPage', () => {
  ...
  it('abort calls running task abort functions and updates status', async () => {
    listSpecs.mockResolvedValue([
      {
        spec_id: 'multi',
        page_id: 'multi',
    ...
    streamAutonomousRun
      .mockReturnValueOnce(abort1)
      .mockReturnValueOnce(abort2);
    ...
    vm.selectAllRunnable(); // selects both
    vm.startBatchRun();

    expect(streamAutonomousRun).toHaveBeenCalledTimes(2);
    ...
    const aborted = Object.values(vm.batchTasks).filter(
      (t) => t.status === 'aborted',
    ).length;
    expect(aborted).toBe(2);
  });

  it('queued tasks are set to aborted on abort without calling abort function', async () => {
    listSpecs.mockResolvedValue([
      {
        spec_id: 'multi',
        page_id: 'multi',
    ...
    aborts.forEach((a) => streamAutonomousRun.mockReturnValueOnce(a));
    ...
    vm.selectAllRunnable(); // selects all 4
    vm.startBatchRun();

    // Only 3 started (concurrency), 1 queued
    expect(streamAutonomousRun).toHaveBeenCalledTimes(3);

    vm.abortRunning();

    // First 3 (running) should have abort called
    expect(aborts[0]).toHaveBeenCalledTimes(1);
    expect(aborts[1]).toHaveBeenCalledTimes(1);
    expect(aborts[2]).toHaveBeenCalledTimes(1);
    // 4th was queued, should NOT have abort called
    expect(aborts[3]).not.toHaveBeenCalled();

    // All 4 should be aborted
    const abortedCount = Object.values(vm.batchTasks).filter(
      (t) => t.status === 'aborted',
    ).length;
